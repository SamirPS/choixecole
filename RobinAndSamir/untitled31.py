...
def alter():
    ...
    connexion.commit()
    curseur.execute("CREATE TABLE EcoleSpe(IdEcole INTEGER PRIMARY KEY AUTOINCREMENT,IdSpe INTEGER,Alternance TEXT)")
    Idspe=""
    lspe=[]
    for i in range(3,r):
        ...
        for i in lspe:
            Idspe=Idspe+" "+str(i)
        curseur.execute("INSERT INTO EcoleSpe (Alternance,IdSpe) VALUES (?,?)",(temp,Idspe))
        lspe=[]
        Idspe=""
