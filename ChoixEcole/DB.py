...
def ecole():
    ...
    connexion.commit()
    curseur.execute("CREATE TABLE `EcoleS` (`Id`	INTEGER PRIMARY KEY AUTOINCREMENT,`Acronyme`	TEXT,`Nom`	TEXT,`Groupe`	TEXT,`Commune`	TEXT,`Region`	TEXT,`Admission`	TEXT,`Statut`	TEXT,`Points`	INTEGER,`PrixB`	INTEGER,`PrixNB`	INTEGER);")
    ...
    for i in range(3,r):
        ...
        for j in range(1, c+1):
            ...
            t2.append(temp)
        curseur.execute("INSERT INTO EcoleS (Acronyme,Nom,Commune,Region,Admission,Statut,Points,PrixB,PrixNB,Groupe) VALUES  (?,?,?,?,?,?,?,?,?,?)",(t2[0],t2[1],t2[2],t2[3],t2[5],t2[6],0,t2[38],t2[39],t2[5]))
