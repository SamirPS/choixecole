...
def ecole():
    ...
    connexion.commit()
    curseur.execute(
        "CREATE TABLE `EcoleS` (`Id`	INTEGER PRIMARY KEY AUTOINCREMENT,`Acronyme`	TEXT,`Nom`	TEXT,`Groupe`	TEXT,`Commune`	TEXT,`Region`	TEXT,`Admission`	TEXT,`Statut`	TEXT,`Points`	INTEGER);"
    )

    for i in range(3, r):

        t2 = [sheet.cell(row=i, column=j).value for j in range(1, c + 1)]
        t2 = pointoecole(t2, Points)
        curseur.execute(
            "INSERT INTO EcoleS (Acronyme,Nom,Commune,Region,Admission,Statut,Points,Groupe) VALUES  (?,?,?,?,?,?,?,?)",
            (t2[0], t2[1], t2[2], t2[3], t2[5], t2[6], t2[-1], t2[5]),
        )
...
def point():
    ...
    while Fini == False:
        ...
        debut = int(input("donne la ligne du début des groupes"))
        ...
        Test = input(
            "avez vous fini avec les barres des groupes alors dites oui sinon appuyer sur une touche et entree'"
        )
        if Test == "oui":
            Fini = True
    return concours
...
def spe():
    ...
    connexion.commit()
    curseur.execute(
        "CREATE TABLE Specialite(Idspecialite INTEGER PRIMARY KEY AUTOINCREMENT,NomSpe TEXT)"
    )
    ...
    for i in range(3, r):
        for j in range(11, 35):
            ...
            if temp2 == "OUI":
                ...
                curseur.execute(
                    "INSERT INTO EcoleSpe (Alternance,IdSpe,IdEcole) VALUES (?,?,?)",
                    (temp, j - 10, i - 2),
                )
